...
import { deleteBlog, handleErrors,handleErrorsInSuccessMode } from "../../../apiFunctions";
import { useRouter } from "next/router";
import { jwtChecker } from "../../usefullFunctions";
import Swal from 'sweetalert2'
import withReactContent from 'sweetalert2-react-content'
import LoadingMiddlePage from "../../loadingMiddlePage";
const DeleteModal = ({
  deletedId,
  changeDeletedId,
  showDeleteModal,
...
const handleDelete=async(id:number)=>{
  ...
  try{
    ...
    if(response.isSuccess===true){
      ...
      router.replace('/panel/blogs-management')
  ...
  return (
    <>
      ...
      <div
        ...
        tabIndex={-1}
        aria-hidden="true"
        className="delete-modal h-full flex items-center justify-center fixed animate__fadeInDown  top-0 left-0 right-0 z-50  w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full"
      >
              ...
              <button className="text-color-alert-2 bg-white hover:bg-gray-100 focus:ring-4 
               rounded-lg border-2 border-gray-200 text-sm font-medium
                px-5 py-2.5 focus:z-10 dark:bg-gray-700 dark:text-gray-300
                 ...
                 onClick={() => {changeDeleteModalVisibility(false);handleDelete(deletedId);} }
                 >
                Delete
              </button>
              <button
